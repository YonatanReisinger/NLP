...
# Patterns that indicate the model considers the question unanswerable
_NO_ANSWER_PATTERNS = [
    r"\bno\s*answer\b",
...
def _is_no_answer_response(text):
    """Return True if *text* signals the question is unanswerable."""
    lowered = text.lower().strip()
    return any(re.search(p, lowered) for p in _NO_ANSWER_PATTERNS)


def _remove_common_prefixes(text):
...
def _is_grounded_in_context(answer, context):
    """
    ...
    content_words = [w for w in norm_answer.split() if w not in _STOPWORDS]
...
def process_answer(raw_model_output, context):
    ...
    if _is_no_answer_response(first_line):
        return NO_ANSWER_MARKER

    # Clean up
    answer = _remove_common_prefixes(first_line)
    ...
    if not _is_grounded_in_context(answer, context):
        return NO_ANSWER_MARKER
